...
serve(async (req) => {
  ...
  try {
    ...
    const groupedPorCategoria = rows.reduce<Record<string, number>>((acc, row) => {
    ...
    return new Response(
      JSON.stringify({
        ...
        valor_vencido: totalVencidas,
        agrupado_categoria: groupedPorCategoria,
        itens: rows,
      }),
      {
        status: 200,
    ...
    console.error('[relatorios-payables] error:', error);
    return new Response(
