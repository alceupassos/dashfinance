-- ========================================================
-- FINANCE ORACULO - BOOTSTRAP MIGRATION V2 (Idempotente)
-- ========================================================
-- Versão corrigida e mais robusta
-- Execute primeiro: select set_config('app.kms', 'B5b0dcf500@#', false);
...
-- DIMENSÃO EMPRESAS
-- =========================
-- Apenas cria índices se a tabela clientes existir
do $$
begin
...
create unique index if not exists idx_dre_unique on dre_entries (company_cnpj, date, account);
...
-- Índice único para evitar duplicatas
create unique index if not exists idx_cf_unique on cashflow_entries (company_cnpj, date, kind, category, amount);
...
-- Tabela intermediária para grupos - SEM foreign key para clientes (mais flexível)
create table if not exists group_alias_members (
  id uuid primary key default gen_random_uuid(),
  group_id uuid not null references group_alias(id) on delete cascade,
  cnpj text not null,  -- Usar CNPJ diretamente ao invés de FK
  cliente_nome text,
  added_at timestamptz default now(),
  unique(group_id, cnpj)
);
...
returns table (
  ...
  with members as (
    select only_digits(cnpj) as cnpj_clean
    from group_alias_members gm
  ...
  base as (
    select date_trunc('month', d.date)::date as month,
           ...
           sum(case when nature='outras'  then amount else 0 end) as outras
    from dre_entries d
    where d.company_cnpj in (select cnpj_clean from members)
...
select cron.unschedule('sync_f360_10min') where exists (select 1 from cron.job where jobname='sync_f360_10min');
select cron.unschedule('sync_omie_15min') where exists (select 1 from cron.job where jobname='sync_omie_15min');
...
select 'Migração concluída com sucesso!' as status,
       (select count(*) from integration_f360) as f360_integracoes,
       (select count(*) from integration_omie) as omie_integracoes,
       (select count(*) from cron.job) as jobs_agendados;
